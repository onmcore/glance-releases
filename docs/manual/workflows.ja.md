...
コミットを右クリック → **Edit history from this commit**（またはブランチ右クリック → **Interactive Rebase…** で、そのブランチ上で編集しながら rebase）すると、上にあるコミットのリストが開き、並べ替え・統合・編集してから再適用できます。ログ内のコミットを直接ドラッグすることもできます — 別の行の端にドロップすれば並べ替え、真ん中にドロップすれば下の行に squash されます。

各コミットに対して以下を選択できます：

- **Pick** — そのまま保持
- **Reword** — 変更は保持し、メッセージだけ編集
- **Squash** — 前のコミットに統合し、両方のメッセージをマージ
- **Fixup** — 前のコミットに統合し、このコミットのメッセージは破棄
- **Drop** — コミットを完全に削除

行をドラッグして並べ替え（またはフォーカスした行で `Alt+↑`/`Alt+↓` を使用）、計画がまだ開いている間は undo/redo（`Ctrl+Z` / `Ctrl+Y`）も可能です。再適用中に競合が発生した場合は、通常の競合解決フローで対応でき、そこから続行または中止できます。

まだサポートしていません：rebase 中のコミット内容の直接編集、カスタムコマンド実行、リポジトリの最初のコミットの rebase、`fixup!`/`squash!` コミットの自動統合、依存ブランチの更新。また、リンク worktree では使用できず、rebase 範囲は最大 1,000 コミットに制限されています。

![ドラッグで並べ替え・squash グループ化できる Interactive Rebase エディタ](assets/screenshots/interactive-rebase.png)
...
リポジトリが Git LFS を使用している場合、**LFS** メニューから **LFS Storage** が開き、LFS がローカルに保存しているすべての内容が分析表示されます：**In use**（現在のチェックアウトで参照中）、**Retained**（何らかのブランチまたはタグでは参照しているが現在のチェックアウトでは参照していない）、**Old versions**（既に上書きされたが履歴を通じてまだ到達可能）、**Orphan**（どこからも参照されていない）に分けたチャートと、容量が大きいフォルダと拡張子別のテーブルが表示されます。

フォルダ行の **Clean** ボタン（または Orphan 全体なら見出しボタン）をクリックするとプレビューが表示され、確認すれば実行されます。これは単なる表示フィルタではなく、古いローカル LFS ファイルを実際に完全削除する操作です — Glance がまずリモートから fetch してリモートにバックアップされていないものは削除しないようにしており、到達可能なすべての内容について最低 1 つのコピーは常に保持します。古いバージョンが後で必要になった場合は、他の欠落 LFS コンテンツと同様にリモートから再ダウンロードされます。

![tier・フォルダ別に分けた LFS Storage の分析](assets/screenshots/lfs-store.png)

### Diff アルゴリズム

特定の diff が期待と異なって表示される場合、**Settings → Editor** で diff アルゴリズムを Histogram（デフォルト）・Myers・Minimal から切り替え可能です。

### コードフォント

**Settings → Appearance** では、インストール済みのシステムフォントからコード・diff 用の等幅フォントをリアルタイムプレビューで選べます。また、サイズ（Small/Normal/Large/XL）はアプリ全体の UI スケールと独立して設定できるため、ボタンやメニューを大きくすることなくコードを読みやすく保つことができます。
