...
- 바이너리 파일은 diff 대신 크기 카드로 표시됩니다. 변경된 이미지 파일은 그 대신 before/after 뷰로 표시됩니다 — 옛 이미지와 새 이미지를 스와이프하거나 onion-skin 슬라이더로 겹쳐 비교할 수 있습니다
...
충돌 한복판에 들어가기 전에 미리 경고해주는 두 기능입니다:

- **충돌 예보(Conflict forecast)** — 각 브랜치(그리고 로그의 ref 라벨)에 배지가 표시되어, 그 브랜치를 기본 브랜치에 머지하면 충돌이 날지를 보여줍니다. 백그라운드에서 계산되며 브랜치가 움직여도 항상 최신 상태를 유지합니다. 배지를 클릭하면 정확히 무엇이 충돌하는지 [리뷰 뷰](#리뷰-뷰)로 열립니다
- **충돌 레이더(Conflict radar)** — 커밋되지 않은 변경사항이 있는 동안, 지금 편집 중인 파일이 최근 활동한 동료의 리모트 브랜치에서도 바뀌었다면 Changes 목록과 커밋 패널에 경고가 뜹니다 — 이미 push해서 엉킨 뒤가 아니라 커밋하기 전에 미리 알 수 있습니다

둘 다 아무것도 막지 않습니다 — 진행을 가로막는 게이트가 아니라 미리 알려주는 경고일 뿐입니다.
...
**Settings → Integrations**에서 GitHub나 GitLab 계정을 연결하면, Glance를 벗어나지 않고 PR/MR을 탐색·리뷰·머지할 수 있습니다.

### 계정 연결

- **GitHub** — Device Flow로 로그인(토큰을 복사해서 붙여넣을 필요 없음), 또는 personal access token 사용
- **GitLab** — 브라우저에서 바로 로그인(토큰 불필요), self-hosted 인스턴스는 personal access token 사용
- self-hosted GitHub Enterprise·GitLab 인스턴스도 지원합니다 — 입력한 호스트에서 provider를 자동으로 판별합니다
- 같은 호스트에 계정 여러 개를 등록할 수 있고, 저장소마다 어떤 계정을 쓸지 기억합니다

### 탐색 & 리뷰

**PR/MR** 탭은 현재 저장소의 열린 PR/MR을 CI 체크 상태와 함께 나열하며, 원하는 주기로(또는 수동으로, 혹은 끄고) 새로고침됩니다.

- PR을 열면 설명, 커밋, CI 체크, 전체 파일 diff를 볼 수 있습니다 — Glance 다른 곳과 동일한 diff 뷰어로 렌더링됩니다
- diff 줄에 직접 코멘트를 달고, 스레드에 답글을 달거나 resolve/unresolve하고, 내가 쓴 코멘트는 수정·삭제할 수 있습니다. 현재 diff와 더 이상 맞지 않는 스레드도 Review 탭 피드에 원래 코드 조각과 함께 남습니다
- Review 탭에서 리뷰를 제출할 수 있습니다 — 승인, 변경 요청, 또는 일반 코멘트. 리뷰 상태(누가 승인했고 누가 아직인지)는 탭을 열지 않아도 PR 헤더에 표시됩니다
- PR/MR 코멘트의 @멘션은 링크로, 이모지 코드는 이모지로 렌더링됩니다

### 생성 & 머지

- PR/MR 탭 상단의 **+** 버튼으로 새 PR을 엽니다 — 소스·타깃 브랜치는 현재 브랜치와 리모트의 기본 브랜치로 기본 설정됩니다
- PR 헤더에서 확인 한 번으로 머지합니다. "소스 브랜치 삭제"는 provider별로 올바르게 처리하며, 서버가 이미 지운 브랜치를 다시 지우려 하지 않습니다
- PR이 타깃보다 뒤처지면 **Update branch** 버튼이 나타납니다

### 최신 상태 유지

멘션과 리뷰 요청은 Glance 알림 센터에 표시되고, 원한다면 Windows 네이티브 토스트로도 뜹니다.
...
**Review** 탭은 현재 체크아웃 상태와 무관하게 임의의 범위를 하나의 연속된 diff로 보여줍니다 — 헤더 칩에서 범위를 고릅니다:

- **커밋되지 않은 변경사항** 또는 **push 안 한 커밋** — upstream 기준(없으면 리모트 기본 브랜치, 그마저 없으면 아무것도 체크아웃 안 된 상태)으로 비교합니다
- **커밋 두 개** — 브랜치·태그·최근 커밋 100개가 나열된 셀렉터에서 기준과 대상을 고르거나, 로그에서 커밋 두 개를 `Shift`+클릭합니다
- 사이드바 파일 목록이 지금 보고 있는 리뷰 내용과 동기화되고, 파일 수가 많으면 기존과 같은 1만 개 상한과 경로 필터가 적용되며 칩으로 해제할 수 있습니다

기존 우클릭 **Compare** 명령을 대체합니다 — 개념은 같고 범위 선택이 더 넓어졌으며, 파일을 하나씩 여는 대신 스테이징과 같은 연속 스크롤 diff 뷰를 공유합니다.
